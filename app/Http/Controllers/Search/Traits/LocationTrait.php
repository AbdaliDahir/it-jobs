...
trait LocationTrait
{
	...
	public function getLocation()
	{
		...
		if (Str::contains(Route::currentRouteAction(), 'Search\SearchController')) {
			if (request()->filled('l') || request()->filled('location')) {
				...
				// dd('hello',$city,request()->get('l'), request()->get('location'), in_array(config('settings.listing.fake_locations_results'), [1, 2]), config('settings'));
	...
	public function getCity($cityId = null, $location = null)
	{
		...
		// /** START MOE-JOBSIGHT */
		$locationSplit =  explode(",", $cityName);
		if (sizeof($locationSplit) > 1) {
			/*$cacheId = md5('city.' . $cityName);
			
			$city = Cache::remember($cacheId, $this->cacheExpiration, function () use ($locationSplit) {
			*/
			$city = City::currentCountry()->Where(function ($query) use ($locationSplit) {
				foreach ($locationSplit as $word) {
					$query->orWhere('name', 'LIKE', '%' . $word . '%');
				}
			})->get();
			// $wordCount = $city->count();
			// if (!empty($city)) {
			// 	$city;
			// }
			/*});*/
