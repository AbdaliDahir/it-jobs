	...
	public function getTitle()
	{
		...
		if (request()->filled('r') && !request()->filled('l')) {
			...
			if (isset($this->city) && !empty($this->city)) {
				...
				// 	$title .= ' ' . $this->city[$key]->name . ',';
				// }

				$title .= '"' . rawurldecode(request()->get('location')) . '"';
				// dd($title, $this->city);
	...
	public function getBreadcrumb()
	{
		...
		if (isset($this->city) && !empty($this->city)) {
			$title = t('in_x_distance_around_city', [
				'distance' => (PostQueries::$distance == 1) ? 0 : PostQueries::$distance,
				'unit'     => getDistanceUnit(config('country.code')),
				'city'     => 'mimid', //$this->city->name,
			]);

			$bcTab[] = collect([
				'name'     => (isset($this->cat) ? t('All jobs') . ' ' . $title : 'mimid'/*$this->city->name*/),
