							...
							<form class="form-horizontal" id="postForm" method="POST"
								...
								<fieldset>
									...
									<div class="form-group row required">
										<label
											class="col-md-3 col-form-label{{ $companyIdError }}">{{ t('Select a Company') }}
											<sup>*</sup></label>
										<div class="col-md-8">
											<select id="companyId" name="company_id"
												class="form-control selecter{{ $companyIdError }}">
												<option value="0" data-logo="" @if (old('company_id', 0)==0)
													selected="selected" @endif>[+] {{ t('New Company') }}</option>
												@if (isset($companies) and $companies->count() > 0)
												@foreach ($companies as $item)
												<option value="{{ $item->id }}"
													data-logo="{{ imgUrl($item->logo, 'small') }}" @if(old('company_id',
													(isset($postCompany) ? $postCompany->id :
													0))==$item->id)
													selected="selected"
													@endif
													>{{ $item->name }}</option>
									...
									@includeFirst([config('larapen.core.customizedViewPath') . 'account.company._form',
									'account.company._form'], ['originForm' => 'post'])
									...
									<div class="form-group row required">
										...
										<div class="{{ $descriptionColClass }}">
											<textarea class="form-control {{ $descriptionError }}" id="description"
												name="description" rows="15">{{ old('description') }}</textarea>
											<small id=""
									...
									<div id="postTypeBloc" class="form-group row required">
										...
										<div class="col-md-8">
											<select name="post_type_id" id="postTypeId"
												class="form-control selecter{{ $postTypeIdError }}">
												@foreach ($postTypes as $postType)
												<option value="{{ $postType->id }}" @if(old('post_type_id')==$postType->
													id)
													selected="selected"
													@endif
											...
											<div class="row">
												<div class="input-group col-md-12">
													...
													<input id="salary_min" name="salary_min"
														...
														placeholder="{{ t('salary_min') }}" type="number" min="0"
												...
												<div class="input-group col-md-12">
													...
													<input id="salary_max" name="salary_max"
														...
														placeholder="{{ t('salary_max') }}" type="number" min="0"
														step="{{ getInputNumberStep((int)config('currency.decimal_places', 2)) }}"
														value="{!! $salaryMax !!}">
													@if (config('currency')['in_left'] == 0)
													<div class="input-group-append">
											...
											<div class="form-check form-check-inline">
												<label class="form-check-label pt-2">
													<input id="negotiable" name="negotiable" type="checkbox" value="1"
														{{ (old('negotiable')=='1') ? 'checked="checked"' : '' }}>&nbsp;{{ t('Negotiable') }}
												</label>
											</div>
										</div>
									</div>
									...
									<div class="form-group row required">
										...
										<div class="col-md-8">
											<input id="address" name="address" placeholder="{{ t('Address') }}"
												class="form-control input-md{{ $addressError }}" type="text"
												value="{{ old('address') }}">
											<small id=""
												class="form-text text-muted">{{ t('Fill an address to display on Google Maps') }}</small>
										</div>
									</div>
									@endif
									*/
									...
									<div class="form-group row required">
										...
										<div class="input-group col-md-8">
											...
											<input id="phone" name="phone" placeholder="{{ t('Phone Number') }}"
											...
											<div class="input-group-append">
												<span class="input-group-text">
													<input name="phone_hidden" id="phoneHidden" type="checkbox"
														value="1"
														{{ (old('phone_hidden')=='1') ? 'checked="checked"' : '' }}>&nbsp;
													<small>{{ t('Hide') }}</small>
												</span>
											</div>
										</div>
									...
									@if (config('country.admin_field_active') == 1 and
									in_array(config('country.admin_type'), ['1', '2']))
									<!-- admin_code -->
									...
									<div class="form-group row">
										<label class="col-md-3 col-form-label"
											for="title">{{ t('Application URL') }}</label>
										<div class="col-md-8">
											<div class="input-group">
												...
												<input id="application_url" name="application_url"
													...
													class="form-control input-md{{ $applicationUrlError }}" type="text"
