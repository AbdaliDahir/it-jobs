			...
			@if ($map['show'])
				...
				$('#countryMap').twism("create",
				{
					...
					click: function(region) { 
                        ...
                        if(setOfRegions.has(region))
                        {
                            setOfRegions.delete(region) ;  
                            var item = document.getElementById(region);
                            $("#locationSearch").tagit("removeTagByLabel", item.getAttribute('name').trim());
			                //divRegions.removeChild(item);       
                        }
                        else
                        {
                            setOfRegions.add(region) ; 
                            var tagSvgReg = document.getElementById(region);
			                tagSvgReg.style.fill = '{{ $map['hover'] }}';
                            /*
			                var a = document.createElement('a');
			                var linkText = document.createTextNode(tagSvgReg.getAttribute('name'));
			                a.appendChild(linkText);
      		                a.title = tagSvgReg.getAttribute('name');
	  		                a.rel="nofollow"
			                a.id="a"+region;
                            ...
                            */
                            $("#locationSearch").tagit("createTagWithId",'tag'+tagSvgReg.id, tagSvgReg.getAttribute('name').trim(),null,true); 
                        }
					...
					hover: function(regionId) {
                        ...
                        for (let i of setOfRegions) {
                            var selectedIdObj = document.getElementById(i);
                        ...
                        /*
                        if (selectedIdObj.classList.contains("active")) {
                            console.log("bypass hover");
							return false;
						}
                        */
						selectedIdObj.style.fill = '{{ $map['hover'] }}';
                        for (i in setOfRegions) {
